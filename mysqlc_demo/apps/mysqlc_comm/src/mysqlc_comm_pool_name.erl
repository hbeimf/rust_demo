...
-define(SYS_CONFIG, mysqlc_comm_pool_config_ets).
-record(mysqlc_comm_pool_config_ets, {
	key,
	val
}).
...
pool_name(ChannelId) ->
	case get_config(ChannelId) of 
		{ok, Val} -> 
			Val;
		_ ->  
			PoolName = lists:concat(["pool_", glib:to_str(ChannelId)]),
			Pn = to_atom(PoolName),
