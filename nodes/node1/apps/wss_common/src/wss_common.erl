...
start_pool(PoolId) ->
  wss_common_pool_sup:start_wss_pool(PoolId).
...
call_all(Cmd, ReqPackage) ->
  Pools = all_pool(),
  call_pool(Pools, Cmd, ReqPackage).
...
cleanup([{Pid, PoolName, _} | Other], PoolName) ->
  ...
  SendPids = lists:map(
    fun({_, WorkPid, _, _}) ->
      gen_server:call(WorkPid, get_send_pid)
    end, Works),
  ?LOG({send_pids, SendPids}),
  poolboy:stop(PoolName),
  erlang:exit(Pid, kill),
  lists:foreach(
    fun(SendPid) ->
      ?LOG(SendPid),
      case erlang:is_pid(SendPid) andalso erlang:is_process_alive(SendPid) of
        true ->
          ?LOG(SendPid),
          SendPid ! stop;
        _ ->
