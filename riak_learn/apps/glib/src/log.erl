...
init([Root, Opts]) ->
    ...
    % ?LOG({Root, Opts, Depth, Chunk, Last, Path}),
    case file(Last) of
        {ok, File} ->
...
do({write, Event}, #state{file=File, path=Path, offs=Offs} = State) ->
    % ?LOG({write, binary_to_term(Event)}),
